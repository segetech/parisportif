...
-- VERSION CORRIGÉE - Supprime les vues conflictuelles d'abord
-- Exécutez ce script dans le SQL Editor de Supabase

-- ============================================
-- 0. SUPPRIMER LES VUES CONFLICTUELLES
-- ============================================
...
CREATE TABLE IF NOT EXISTS operation_history (
  ...
  operation_type VARCHAR(20) NOT NULL, -- 'bets', 'transactions', 'venues'
...
CREATE INDEX IF NOT EXISTS idx_operation_history_operation ON operation_history(operation_type, operation_id);
CREATE INDEX IF NOT EXISTS idx_operation_history_modified_by ON operation_history(modified_by);
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  operation_type VARCHAR(20), -- 'bets', 'transactions', 'venues'
  operation_id UUID,
...
CREATE INDEX IF NOT EXISTS idx_operation_comments_operation ON operation_comments(operation_type, operation_id);
CREATE INDEX IF NOT EXISTS idx_operation_comments_user ON operation_comments(user_id);
CREATE INDEX IF NOT EXISTS idx_operation_comments_created_at ON operation_comments(created_at);
...
BEGIN
  ...
  IF OLD.validation_status IS DISTINCT FROM NEW.validation_status AND NEW.created_by IS NOT NULL THEN
    PERFORM create_notification(
      NEW.created_by,
...
DROP POLICY IF EXISTS "Authenticated users can read history" ON operation_history;
DROP POLICY IF EXISTS "Authenticated users can insert history" ON operation_history;
...
DROP POLICY IF EXISTS "Authenticated users can read comments" ON operation_comments;
DROP POLICY IF EXISTS "Authenticated users can create comments" ON operation_comments;
...
WHERE table_name IN ('validation_stats', 'unread_notifications_count')
ORDER BY table_name;
