...
function UsersTable() {
  ...
  function exportCsv() {
    ...
    const lines = rows.map((r) => [
      ...
      r.role,
      r.statut,
      r.derniere_connexion || "",
      r.cree_le,
    ]);
    const csv = [headers, ...lines]
      .map((l) =>
        l.map((x) => `"${String(x).split('"').join('""')}"`).join(","),
      )
      .join("\n");
  ...
  async function openUserAudit(u: User) {
    ...
    const filtered = all.filter(
      (e) =>
        e.entityId === u.id ||
        (e.details &&
          (e.details.userId === u.id || e.details.user?.id === u.id)),
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-end">
        ...
        <div className="flex flex-col">
          ...
          <select
            ...
            value={statut}
            onChange={(e) => {
              setStatut(e.target.value as any);
              setPage(1);
            }}
          >
            <option value="TOUS">Tous</option>
        ...
        <table className="w-full text-sm min-w-[900px]">
          ...
          <tbody>
            {rows.length ? (
                ...
                <tr key={r.id} className="border-t">
                  ...
                  <td className="p-2">
                    <span
                      className={
                        r.role === "ADMIN"
                          ? "px-2 py-0.5 text-xs rounded bg-fuchsia-100 text-fuchsia-700"
                          : r.role === "CONTROLEUR"
                      ...
                      className={
                        r.statut === "actif"
                          ...
                          : r.statut === "suspendu"
                            ...
                            : r.statut === "invitation_envoyee"
                              ? "px-2 py-0.5 text-xs rounded bg-orange-100 text-orange-700"
                              : "px-2 py-0.5 text-xs rounded bg-slate-100 text-slate-700"
                      }
                    >
                      {r.statut === "invitation_envoyee"
                  ...
                  <td className="p-2" title={r.derniere_connexion || ""}>
                    {r.derniere_connexion
                      ? new Intl.DateTimeFormat("fr-FR", {
                          dateStyle: "short",
                          timeStyle: "short",
                        }).format(new Date(r.derniere_connexion))
                      : "—"}
                  </td>
                  <td className="p-2">
                    {new Intl.DateTimeFormat("fr-FR", {
                      dateStyle: "short",
                    }).format(new Date(r.cree_le))}
                  </td>
                  <td className="p-2">
                    <div className="flex gap-2">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="start">
                          <DropdownMenuItem onClick={() => setProfileOpen(r)}>
                            Voir le profil
                          </DropdownMenuItem>
                          ...
                          {isAdmin && (
                            <DropdownMenuItem onClick={() => setRoleOpen(r)}>
                              Changer le rôle
                            </DropdownMenuItem>
                          )}
                          {isAdmin && (
                            <DropdownMenuItem
                              onClick={() => setResetOpen({ user: r, url: "" })}
                            >
                          ...
                          {isAdmin &&
                            (r.statut !== "suspendu" ? (
                              <DropdownMenuItem
                                onClick={() => setSuspendOpen(r)}
                              >
                                Suspendre…
                              </DropdownMenuItem>
                            ) : (
                              <DropdownMenuItem onClick={() => reactivate(r)}>
                                Réactiver
                              </DropdownMenuItem>
                            ))}
                          {isAdmin && (
                          ...
                          <DropdownMenuItem
                            onClick={() =>
                              (window.location.href = `/journal?user=${encodeURIComponent(r.id)}`)
                            }
                          >
                            Voir l��historique
                          </DropdownMenuItem>
              ...
              <tr>
                <td
                  className="text-center text-sm text-muted-foreground p-3"
                  colSpan={6}
                >
            ...
            {!invTempPassword ? (
              <>
                ...
                <div>
                  ...
                  <Input
                    value={invPrenom}
                    onChange={(e) => setInvPrenom(e.target.value)}
                    disabled={invLoading}
                  />
                </div>
                <div>
                ...
                <div>
                  <label className="text-xs font-medium">
                    Méthode de transmission du mot de passe
                  </label>
                  <select
                    className="border rounded px-2 py-2 text-sm w-full"
                    value={invMethod}
                    onChange={(e) =>
                      setInvMethod(e.target.value as "email" | "direct")
                    }
                    disabled={invLoading}
                  >
              ...
              <>
                ...
                <div>
                  ...
                  <div className="border rounded p-3 bg-muted/30 font-mono text-sm break-all">
                    {invTempPassword}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
                    L'utilisateur doit changer ce mot de passe à sa première
                    connexion.
                  </p>
                </div>
      ...
      <Sheet
        open={!!profileOpen}
        onOpenChange={(o) => !o && setProfileOpen(null)}
      >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-lg font-semibold">
                    {profileOpen.nom}
                    {profileOpen.prenom ? ` ${profileOpen.prenom}` : ""}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <div>
                    <span
                      className={
                        profileOpen.role === "ADMIN"
                          ? "px-2 py-0.5 text-xs rounded bg-fuchsia-100 text-fuchsia-700"
                          : profileOpen.role === "CONTROLEUR"
                            ? "px-2 py-0.5 text-xs rounded bg-blue-100 text-blue-700"
                            : "px-2 py-0.5 text-xs rounded bg-slate-100 text-slate-700"
                      }
                    >
                ...
                <div>
                  ...
                  <div>
                    <span
                      className={
                        profileOpen.statut === "actif"
                          ? "px-2 py-0.5 text-xs rounded bg-emerald-100 text-emerald-700"
                          : profileOpen.statut === "suspendu"
                            ? "px-2 py-0.5 text-xs rounded bg-red-100 text-red-700"
                            : profileOpen.statut === "invitation_envoyee"
                              ? "px-2 py-0.5 text-xs rounded bg-orange-100 text-orange-700"
                              : "px-2 py-0.5 text-xs rounded bg-slate-100 text-slate-700"
                      }
                    >
                      {profileOpen.statut === "invitation_envoyee"
                        ? "Invitation envoyée"
                        : profileOpen.statut === "desactive"
                          ? "Désactivé"
                          : profileOpen.statut.charAt(0).toUpperCase() +
                            profileOpen.statut.slice(1)}
                    </span>
                  </div>
                </div>
                <div>
                  ...
                  <div title={profileOpen.derniere_connexion || ""}>
                    {profileOpen.derniere_connexion
                      ? new Intl.DateTimeFormat("fr-FR", {
                          dateStyle: "short",
                          timeStyle: "short",
                        }).format(new Date(profileOpen.derniere_connexion))
                      : "—"}
                  </div>
                </div>
                <div>
                  ...
                  <div>
                    {new Intl.DateTimeFormat("fr-FR", {
                      dateStyle: "short",
                      timeStyle: "short",
                    }).format(new Date(profileOpen.cree_le))}
                  </div>
                </div>
                <div>
                  ...
                  <div>
                    {new Intl.DateTimeFormat("fr-FR", {
                      dateStyle: "short",
                      timeStyle: "short",
                    }).format(new Date(profileOpen.mis_a_jour_le))}
                  </div>
                </div>
                <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {isAdmin &&
                  (profileOpen.statut !== "suspendu" ? (
                    ...
                    <Button
                      variant="outline"
                      onClick={() => reactivate(profileOpen)}
                    >
                      Réactiver
                    </Button>
                  ))}
                {isAdmin && (
                  <Button
                    ...
                    onClick={() => {
                      setProfileOpen(null);
                      setResetOpen({ user: profileOpen, url: "" });
                    }}
                  >
                    Reset MDP
                  </Button>
                ...
                <Button
                  variant="outline"
                  onClick={() => openUserAudit(profileOpen)}
                >
                  Voir l’historique
                </Button>
              </div>
        ...
        <DialogContent>
          ...
          <div className="grid gap-3">
            ...
            <div>
              ...
              <Input
                value={edPrenom}
                onChange={(e) => setEdPrenom(e.target.value)}
              />
            </div>
            <div>
              <label className="text-xs font-medium">Email</label>
            ...
            <div>
              ...
              <select
                className="border rounded px-2 py-2 text-sm w-full"
                value={edRole}
                onChange={(e) => setEdRole(e.target.value as Role)}
                disabled={!isAdmin}
              >
                <option value="ADMIN">Admin</option>
                <option value="CONTROLEUR">Contrôleur</option>
            ...
            <div>
              ...
              <select
                className="border rounded px-2 py-2 text-sm w-full"
                value={edStatut}
                onChange={(e) => setEdStatut(e.target.value as UserStatus)}
              >
                <option value="actif">Actif</option>
                <option value="suspendu">Suspendu</option>
                <option value="invitation_envoyee">Invitation envoyée</option>
            ...
            <div className="text-xs text-muted-foreground">
              MFA activée: {editOpen?.mfa_active ? "Oui" : "Non"}
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" onClick={() => setEditOpen(null)}>
                Annuler
              </Button>
              <Button onClick={editSubmit} disabled={!isAdmin}>
                Enregistrer
              </Button>
            </div>
          </div>
          ...
          <div className="grid gap-3">
            <div>
              ...
              <select
                className="border rounded px-2 py-2 text-sm w-full"
                value={crRole}
                onChange={(e) => setCrRole(e.target.value as Role)}
              >
                <option value="ADMIN">Admin</option>
                <option value="CONTROLEUR">Contrôleur</option>
                <option value="AGENT">Agent</option>
              ...
              <div className="text-xs text-muted-foreground mt-1">
                Attention: Au moins un Administrateur doit rester actif.
              </div>
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" onClick={() => setRoleOpen(null)}>
                Annuler
              </Button>
              <Button onClick={changeRoleSubmit} disabled={!isAdmin}>
                Mettre à jour
              </Button>
            </div>
          </div>
      ...
      <Dialog
        open={!!suspendOpen}
        onOpenChange={(o) => !o && setSuspendOpen(null)}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="grid gap-3">
            <div>
              ...
              <textarea
                className="border rounded px-2 py-1 text-sm w-full min-h-[100px]"
                value={susMotif}
                onChange={(e) => setSusMotif(e.target.value)}
              />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" onClick={() => setSuspendOpen(null)}>
                Annuler
              </Button>
              <Button onClick={suspendSubmit} disabled={!isAdmin}>
                Suspendre
              </Button>
            </div>
          </div>
      ...
      >
        <DialogContent>
          ...
          <div className="grid gap-3">
            <div className="text-sm">
              Cette action désactivera le compte (suppression logique).
            </div>
            <div>
              <label className="text-xs font-medium">Motif</label>
              <textarea
      ...
      <Dialog
        open={!!resetOpen?.user && resetOpen?.url === ""}
        onOpenChange={(o) => {
          if (!o) {
            setResetOpen(null);
      ...
      >
        <DialogContent>
          ...
          <div className="grid gap-3">
            <div className="text-sm">
              Comment voulez-vous transmettre le nouveau mot de passe?
            </div>
            <div className="flex gap-2">
              <Button
      ...
      <Dialog
        open={!!resetOpen?.url}
        onOpenChange={(o) => !o && setResetOpen(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
